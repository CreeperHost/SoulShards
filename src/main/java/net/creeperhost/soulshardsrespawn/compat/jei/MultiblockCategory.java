...
public class MultiblockCategory implements IRecipeCategory<MultiblockCategory.Recipe>
{
    ...
    public MultiblockCategory(IGuiHelper iGuiHelper)
    {
        ...
        arrowRight = iGuiHelper.drawableBuilder(Identifier.withDefaultNamespace("textures/gui/container/furnace.png"), 176, 14, 24, 17).build();
    }
    ...
    public void draw(Recipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor guiGraphics, double mouseX, double mouseY)
    {
        ...
        for (FormattedCharSequence line : craftLabel) {
            guiGraphics.text(font, line, 2, y, 0, false);
            y += 10;
        }
    }
    ...
    {
        builder.addSlot(RecipeIngredientRole.INPUT, 23, 1).add(recipe.getInput());

        builder.addSlot(RecipeIngredientRole.OUTPUT, 90, 30).add(recipe.getOutput());
    }
